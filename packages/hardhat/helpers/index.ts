...
import { Erc20, GardensTemplate, IUnipoolFactory } from '../typechain'
import { eEthereumNetwork, eXDaiNetwork } from '../utils/types'

export const getGardensTemplate = async (signer: Signer): Promise<GardensTemplate> => {
  const gardensTemplateDeployment = await deployments.get('GardensTemplate')
  return (await ethers.getContractAt('GardensTemplate', gardensTemplateDeployment.address, signer)) as GardensTemplate
}
...
export const getHoneyToken = async (signer: Signer, gardensTemplate: GardensTemplate): Promise<Erc20> => {
...
export const getOriginalToken = async (signer: Signer, address: string): Promise<Erc20> => {
