...
export const calculateRewards = async (
  ...
  return balance
    ...
    .div(String(1e18))
    .mul(totalRaised)
    .div(totalRaised.add(expectedRaise));
};
...
export async function claimRewards(
  ...
  const total = Math.ceil(
    contributors.length / CONTRIBUTORS_PROCESSED_PER_TRANSACTION
  );
  let counter = 1;
  let tx;
  ...
  for (
    ...
    tx = await impactHours.claimReward(
    ...
    log(
      `Tx ${counter++} of ${total}: Rewards claimed for IH token holders ${
        i + 1
      } to ${Math.min(
        i + CONTRIBUTORS_PROCESSED_PER_TRANSACTION,
        contributors.length
...
export const log = (message: string, spaces = 4): void =>
